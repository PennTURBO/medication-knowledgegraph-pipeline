...
pds.generic_name.approximate$query.source <- "lowercased GENERIC_NAME"
...
# could persue training on relations between query and search result terms here,
#   as an alternative to exact amtch true/false
...
dput(sort(setdiff(all.cols, accounted.cols)))
dput(sort(setdiff(accounted.cols, all.cols)))
sort(table(accounted.cols))
...
# [, c(factor.predictors, numeric.predictors)]
...
uncovered.frame <- performance.frame[performance.frame$FK_MEDICATION_ID %in% uncovered.keys , ]
